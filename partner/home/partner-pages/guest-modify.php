<?
	...
	$sql = "SELECT * FROM guest_user_t WHERE partner_id = '{$db_partner_id}' AND guest_id = '{$db_guest_id}'";
...
?>
<div>
	<t3 style='height:40px; padding-top:20px'>고객사 정보 변경</t3>
	<hr>
	...
	<tbody>
		...
		?>
			<tr>
				...
				<td><a href='#' onclick='<?=$js_page_id?>.action.on_btn_delete_app("<?=$row['app_key']?>", "<?=$row['app_title']?>")' data-role='button' data-mini='true' data-inlnie='true'>삭제</a></td>
			</tr>
		<?
		}
		?>
	...
	{			
		action: {
			...
			on_btn_modifyguest: function()
			{
				...
				util.post(get_ajax_url('partner-guest-modify'), ar_param, function(sz_data) {
			...
			on_select_appkey: function(appkey, apptitle)
			{
				...
				util.MessageBox('알림', '"' + apptitle + '" 를 추가하겠습니까 ?', function(sel) {
					if (sel == 1) {

						util.post(get_ajax_url('partner-guest-add-appkey'), ar_param, function(sz_data) {
							var js_data = util.to_json(sz_data);
							if (js_data['result']) {
								util.Alert('알림', '추가되었습니다.', function() {
									window.location.reload();
								});	
							} else util.Alert(js_data['msg']);
						});
			...
			on_btn_delete_app: function(appkey, apptitle)
			{
				var ar_param = {
					'guestid' : '<?=$guest_id?>',
					'appkey' : appkey,
				...
				util.MessageBox('알림', '"' + apptitle + '" 를 삭제하겠습니까 ?', function(sel) {
					if (sel == 1) {
						...
						util.post(get_ajax_url('partner-guest-del-appkey'), ar_param, function(sz_data) {
							var js_data = util.to_json(sz_data);
							if (js_data['result']) {
