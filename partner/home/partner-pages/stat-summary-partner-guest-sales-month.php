<?
	...
	// test : $sql = "SELECT b.guest_id, SUM(a.publisher_cnt) AS 'cnt', SUM(a.publisher_fee) AS 'fee', reg_day
	$sql = "SELECT b.guest_id, SUM(a.merchant_cnt) AS 'cnt', SUM(a.merchant_fee) AS 'fee', reg_day
			...
			WHERE b.guest_id = '{$db_guest_id}' AND a.reg_day >= '{$year}-{$month}-01' AND a.reg_day <= LAST_DAY('{$year}-{$month}-01')
			GROUP BY a.reg_day";
