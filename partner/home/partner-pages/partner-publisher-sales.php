<?
	...
	// ---------------------------------------
	$sql = "SELECT SUM(c.publisher_cnt) as 'publisher_cnt', SUM(c.publisher_fee) as 'publisher_fee'
	...
	$row = mysql_fetch_assoc(mysql_query($sql, $conn));
	$total_publisher_cnt = $row['publisher_cnt'];
	$total_publisher_fee = $row['publisher_fee'];
...
<div style='width:800px'>
	...
	<tbody>
		<tr class='sum'>
			<td colspan=5 style='text-align:left; padding-left:10px'>전체 합</td>
			<td><?=admin_number($total_publisher_cnt)?></td>
			<td><?=admin_number($total_publisher_fee)?></td>
		</tr>
	<?
		$idx = 0;
		...
		while ( $publisher = mysql_fetch_assoc($result) ) {
			...
			<tr style='cursor:pointer' id='line-m-<?=$publisher['pcode']?>' class="mactive-<?=$publisher['is_mactive']?>">
				...
				<td <?=$td_onclick?>><b><?=admin_number($publisher['publisher_cnt'])?></b></td>
				<td <?=$td_onclick?>><b><?=admin_number($publisher['publisher_fee'])?></b></td>
			</tr>
			<?
		}
