<?
	...
	// guest sales info
	// ---------------------------------------
	// test : $sql = "SELECT a.guest_id, a.guest_name, SUM(c.publisher_cnt) AS 'merchant_cnt', SUM(c.publisher_fee) AS 'merchant_fee'
	$sql = "SELECT a.guest_id, a.guest_name, SUM(c.merchant_cnt) AS 'merchant_cnt', SUM(c.merchant_fee) AS 'merchant_fee'
			FROM guest_user_t a
				LEFT OUTER JOIN guest_app_list b ON a.guest_id = b.guest_id
				LEFT OUTER JOIN al_summary_sales_m_t c ON b.app_key = c.app_key AND c.reg_day = '{$year}-{$month}-01'
			WHERE a.partner_id = '{$db_partner_id}' {$where}
			GROUP BY a.guest_id {$order_by} {$limit}";
...
<div style='width:800px'>
	<t4 style='line-height: 40px'>광고주 실적</t4>
	<hr>
	...
	<form onsubmit='return <?=$js_page_id?>.action.on_btn_search()'>
		...
		<tr><td id='btns-group' valign=top>
			...
			<div style='width:450px; padding-top:0px; text-align: left'>
			    ...
			    <div class='ui-grid-b' style='padding:2px 0px; width: 450px; margin: 0 0 0 auto'>
			    	<div class='ui-block-a' style='display: block; width:230px'>
			    	...
			    	<div class='ui-block-b' style='width:150px'><input type=text name=search id=search data-clear-btn='true' value="<?=$_REQUEST['search']?>"  style='line-height: 25px;'/></div>
	...
	<?
		...
		while ( $row = mysql_fetch_assoc($result) ) {
			...
			<tr style='cursor:pointer' id='line-m-<?=$row['guest_id']?>'>
				...
				<td <?=$td_onclick?>><?=$row['guest_name']?></td>
				<td <?=$td_onclick?>><?=$row['guest_id']?></td>
				<td <?=$td_onclick?>><b><?=admin_number($row['merchant_cnt'])?></b></td>
				<td <?=$td_onclick?>><b><?=admin_number($row['merchant_fee'])?></b></td>
			</tr>
			<?
		}
