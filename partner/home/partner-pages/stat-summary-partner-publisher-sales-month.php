<?
	...
	$sql = "SELECT a.pcode, sum(a.publisher_cnt) as 'cnt', sum(a.publisher_fee) as 'fee', reg_day, b.name 
			FROM al_summary_sales_d_t a 
				INNER JOIN al_publisher_t b ON a.pcode = b.pcode 
			WHERE a.pcode = '{$db_pcode}' AND a.reg_day >= '{$year}-{$month}-01' AND a.reg_day <= LAST_DAY('{$year}-{$month}-01') 
			GROUP by a.reg_day";

	$result = mysql_query($sql, $conn);
	while ($row = mysql_fetch_assoc($result)) {

		// pcode.name 이 하나만 존재하며 ==> 이 구성 그대로 두되, 목록 UI에서 합 컬럽만 제거한다.
		...
		<div class='ui-block-a'>
			...
			<div style="display:inline-block">
				<div class='ui-grid-a'>
					<!-- 년도 선택 -->
					<!--; -->
					<div class='ui-block-a' style='width:100px; padding-top:5px'><input type="text" data-role='text' id='param-date' style='text-align: center' value="<?=$year . '-' . $month?>" /></div>
...
<?
		for ($i = 1; $i <= $last_day; $i ++)
		{
			...
			?>
			<tr class='row-datum week-<?=$week_day?>' onclick=window.location.href='?id=stat-summary-partner-publisher-app-sales-month&partnerid=<?=$partner_id?>&pcode=<?=$pcode?>&date=<?=$check_date?>' style='cursor:pointer'>
				<td><?=$i?>일</td>
