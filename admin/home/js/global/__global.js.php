...
function get_ajax_url(ajax_id, param) {
	var url = "";
	url = "http://<?=$_SERVER['HTTP_HOST']?>/ajax-request.php?id=" + ajax_id + '&admin_id=' + util.urlencode('<?=$_SESSION['adminid']?>') + '&umcode=' + util.urlencode('<?=$_SESSION['umcode']?>');
	...
	setTimeout(function(){
		...
		if (page_id != $.mobile.activePage.attr('id')) 
		{
			...
			if ($('#'+page_id).length == 0)
			{
				...
				// 처음 페이지 호출시
				util.request("<?=$_SERVER['REQUEST_SCHEME']?>://<?=$_SERVER['HTTP_HOST']?>/admin_index_ajax.php?page_id="+util.urlencode(page_id) + sz_webparam, function(sz_data) {
					try {
