<?
	...
	$sql = "SELECT p.*, pa.merchant_disabled, pa.merchant_enabled, IFNULL(IF('$is_public_mode' = 'Y', pa.merchant_disabled, pa.merchant_enabled), 'N') as 'checked', t.name as 'partner_name', t.company as 'partner_company', SUM(s.merchant_cnt) as 'merchant_cnt'
			FROM al_publisher_t p
				LEFT OUTER JOIN al_publisher_app_t pa ON p.pcode = pa.pcode AND pa.app_key = '{$db_appkey}'
					...
					LEFT OUTER JOIN al_summary_sales_h_t s ON s.mcode = '{$db_mcode}' AND s.pcode = p.pcode AND s.reg_day = CURRENT_DATE AND s.app_key = '{$db_appkey}'
	...
	<?
		...
		while ($publisherapp = mysql_fetch_assoc($result)) {
			...
			<tr style=''>
				...
				<td><?=admin_number($publisherapp['merchant_cnt'])?></td>
