<?
	...
	$sql = "SELECT a.mcode, sum(a.merchant_cnt) as 'cnt', sum(a.merchant_fee) as 'fee', reg_day, b.name FROM al_summary_sales_m_t a INNER JOIN al_merchant_t b ON a.mcode = b.mcode WHERE reg_day >= '{$year}-01-01' AND reg_day <= '{$year}-12-31' GROUP by a.mcode, reg_day";
		...
		<div class='ui-block-a'>
			...
			<div style="display:inline-block">
				<div class='ui-grid-a'>
					<!-- 년도 선택 -->
					<!--; -->
					<div class='ui-block-a' style='width:120px; padding-top:5px'><input type="text" id='param-date' data-clear-btn='true' value="<?=$year?>" /></div>
					<div class='ui-block-b' style='width:300px; padding-left:5px'>
						<a href='#' onclick="$('#param-date').val('<?=$year - 1 ?>').trigger('change'); return false;" data-role='button' data-mini='true' data-inline='true'><<</a>
						<a href='#' onclick="$('#param-date').val('<?=date("Y")?>').trigger('change'); return false;" data-role='button' data-mini='true' data-inline='true'>올해</a>
						<a href='#' onclick="$('#param-date').val('<?=$year + 1 ?>').trigger('change'); return false;" data-role='button' data-mini='true' data-inline='true'>>></a>
					</div>
				</div>
				<script>$('#param-date').change(function(){ window.location.href=g_admin_util.set_url_param(location.href, "date", $(this).val() + "-01-01"); });</script>
			</div>
		</div>
...
<?
        	foreach($ar_key_names as $key => $val) {
	        	echo "\n<th colspan=2 width=120px><a href='?id=stat-summary-mp-sales-year&mcode={$val}'>{$key}</a></th>";
        	}
?>			
...
<?
		for ($i = 1; $i <= 12; $i ++)
		{
			...
			?>
			<tr onclick=window.location.href='?id=stat-summary-m-sales-month&date=<?=$check_date?>' style='cursor:pointer'>
				<td><?=$i?>월</td>
