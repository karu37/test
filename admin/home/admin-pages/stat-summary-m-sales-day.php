<?
	...
	$sql = "SELECT a.mcode, sum(a.merchant_cnt) as 'cnt', sum(a.merchant_fee) as 'fee', reg_day, hr, b.name FROM al_summary_sales_h_t a INNER JOIN al_merchant_t b ON a.mcode = b.mcode WHERE reg_day = '{$year}-{$month}-{$day}' GROUP by a.mcode, reg_day, hr";
...
?>
	...
	<div class='ui-grid-a'>
		...
		<div class='ui-block-b' style='text-align:right'>
			<a href='?id=stat-summary-m-sales-year&date=<?=$date?>' data-role='button' data-inline='true' data-mini='true'>연간 매출 현황</a>
			<a href='?id=stat-summary-m-sales-month&date=<?=$date?>' data-role='button' data-inline='true' data-mini='true'>월간 매출 현황</a>
			<a href='?id=stat-summary-m-sales-day&date=<?=$date?>' data-role='button' data-inline='true' data-mini='true'>일간 매출 현황</a>
		</div>
	</div>
	<hr>
	<br>
...
<?
        	foreach($ar_key_names as $key => $val) {
	        	echo "\n<th colspan=2 width=120px><a href='?id=stat-summary-mp-sales-day&mcode={$val}'>{$key}</a></th>";
        	}
?>			
