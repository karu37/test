<?
	...
	$sql = "SELECT COUNT(*) as cnt FROM guest_user_t a WHERE 1=1 {$where}";
	$row = mysql_fetch_assoc(mysql_query($sql, $conn));
	$pages = new Paginator($row['cnt']);
	$limit = "LIMIT " . $pages->limit_start . "," . $pages->limit_end;
	// --------------------------------

	$sql = "SELECT type, name FROM store_category_t";
...
?>
	...
	<t4 style='line-height: 40px'>광고주 접속 계정 목록</t4>
	<hr>
	...
	<div style='padding: 10px'>
  			<a href='?id=guest-regist' data-role='button' data-mini='true' data-inline='true'>새 고객사 등록</a>
	</div>		
	<hr>
	<br>
