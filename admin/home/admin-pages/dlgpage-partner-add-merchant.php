<?
	...
	else if ($searchfor == "code" && $search) $where .= " AND m.mcode LIKE '%{$db_search}%'";

	// --------------------------------
	// Paginavigator initialize	
	$sql = "SELECT COUNT(*) as cnt FROM al_merchant_t m LEFT OUTER JOIN al_partner_mpcode_t mp ON m.mcode = mp.mcode AND mp.partner_id = '{$db_partner_id}' AND TYPE = 'M' WHERE mp.mcode IS NULL {$where}";
	$row = mysql_fetch_assoc(mysql_query($sql, $conn));
	$pages = new Paginator($row['cnt']);
	$limit = "LIMIT " . $pages->limit_start . "," . $pages->limit_end;
	// --------------------------------
	$sql = "SELECT m.* FROM al_merchant_t m 
				LEFT OUTER JOIN al_partner_mpcode_t mp ON m.mcode = mp.mcode AND mp.partner_id = '{$db_partner_id}' AND TYPE = 'M'
			WHERE mp.mcode IS NULL {$where}
...
<div style='width:700px'>	
	<t4 style='line-height: 40px'><b3 style='color:darkred'><?=$partner_name?></b3>에 Merchant 추가하기</t4>
	<hr>
		...
		<tr><td id='btns-group' valign=top>
			...
			<div style='width:300px; padding-top:10px; text-align: left'>
				<fieldset id="search-for" class='td-2-item' data-role="controlgroup" data-type="horizontal" style='margin-top: 3px;' data-mini=true init-value="<?=$searchfor?>" >
			        ...
			        <label for="search-for-name">Merchant 명</label>
			        <input name="search-for" id="search-for-code" value="code" type="radio" />
			        <label for="search-for-code">Merchant 코드</label>
			    </fieldset>	
	...
	<tbody>
	<?
		while ($merchantapp = mysql_fetch_assoc($result)) {
		
			?>
			<tr style='' id='line-<?=$merchantapp['mcode']?>'>
				<td><?=$merchantapp['name']?></td>
				<td><?=$merchantapp['mcode']?></td>
				<td class='btn-td'>
					<a href='#' onclick='<?=$js_page_id?>.action.on_btn_set_merchant_add("<?=$partner_id?>", "<?=$merchantapp['mcode']?>")' data-theme='<?=$ar_btn_theme[1]?>'  data-role='button' data-mini='true' data-inline='true'>추가</a>
				</td>
			</tr>
			<?
		}
...
var <?=$js_page_id?> = function()
{
	...
	{			
		action: {
			...
			on_btn_set_merchant_add: function(partner_id, mcode) {

				var ar_param = {partnerid: partner_id, 'mcode': mcode};
				util.request(get_ajax_url('admin-partner-add-merchant', ar_param), function(sz_data) {
