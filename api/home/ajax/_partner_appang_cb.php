...
/* ******************************************
	// callback base url : http://cb.aline-soft.kr/ajax-request.php?id=_partner_appang_cb
	// "http://cb.aline-soft.kr/ajax-request.php?id=_partner_appang_cb&appkey=apgca56209ce2b04fc6e7288375b9f689bb&uniquekey=apguniquekey&userdata=" . base64_encode(json_encode(array('aid' => $userapp_id)))
**********************************************/
	...
	//## unique_key
	$unique_key 	= $_REQUEST['s'];			// 안주는 Merchant 의 경우 ==> $g_appang['unique_prefix'] . md5($_REQUEST['ak'] . $_REQUEST['ud']);
	
	//## userdata ==> aid
	$cb_user_data 	= $_REQUEST['ud'];
	...
	if ($ar_reward['callback_done'] != 'Y') {
		...
		mysql_close($conn);
		$response_data = post($req_base_url, $url_param, $g_appang['timeout_sec']);
		$conn = dbConn();
		
		$ar_resp = json_decode($response_data, true);
		
		make_action_log("callback-pub-appang", ifempty($ar_resp['result'], 'N'), $arr_userapp['pcode'], $arr_userapp['adid'], $arr_userapp['app_key'], null, get_timestamp() - $start_tm, $req_base_url, $url_param, $response_data, $conn);
