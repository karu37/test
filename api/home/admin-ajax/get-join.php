<?
	...
	*/
	...
	$os = $_REQUEST['os'];			// 2개 모두 지원하는 경우에 참고로만 사용함
	...
	if (!$os || !$pcode || !$appkey || !$uid || !$adid || !$ip) return_die('N', array('code'=>'-101', 'type'=>'E-REQUEST'), '파라미터 오류입니다..');
	
	if ( !in_array($os, array('I', 'A')) )  return_die(false, array('code'=>'-102', 'type'=>'E-REQUEST'), '파라미터 코드 값 오류입니다. (os)');
	...
	if ($row_userapp) {
		...
		$sql = "INSERT al_user_app_t (mcode, pcode, app_key, adid, ip, uid, userdata, fee_merchant, fee_publisher, action_atime, status, reg_day, reg_date)
